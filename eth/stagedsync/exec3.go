			...
			for outputTxNum.Load() <= maxTxNum {
				...
				case <-pruneEvery.C:
					...
					if sz < commitThreshold {
						...
						// non-aggressive mode: flush history because commit is soon - and will flush anyway
						commitIsSoon := sz > uint64(float64(commitThreshold)*0.8)
						if commitIsSoon {
