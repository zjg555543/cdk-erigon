...
func NewStagedSync(ctx context.Context,
	...
	return stagedsync.New(
		stagedsync.DefaultStages(ctx, stagedsync.StageHeadersCfg(
			db,
			controlServer.Hd,
			controlServer.Bd,
			*controlServer.ChainConfig,
			...
			notifications,
			forkValidator), stagedsync.StageCumulativeIndexCfg(db), stagedsync.StageBlockHashesCfg(db, dirs.Tmp, controlServer.ChainConfig), stagedsync.StageBodiesCfg(
			db,
			controlServer.Bd,
			...
			*controlServer.ChainConfig,
			...
			cfg.HistoryV2,
			txNums,
		), stagedsync.StageIssuanceCfg(db, controlServer.ChainConfig, blockReader, cfg.EnabledIssuance), stagedsync.StageSendersCfg(db, controlServer.ChainConfig, false, dirs.Tmp, cfg.Prune, blockRetire, controlServer.Hd), stagedsync.StageExecuteBlocksCfg(
			db,
			cfg.Prune,
			cfg.BatchSize,
			nil,
			...
			txNums,
			agg,
		), stagedsync.StageHashStateCfg(db, dirs, cfg.HistoryV2, txNums, agg), stagedsync.StageTrieCfg(db, true, true, false, dirs.Tmp, blockReader, controlServer.Hd, cfg.HistoryV2, txNums, agg), stagedsync.StageHistoryCfg(db, cfg.Prune, dirs.Tmp), stagedsync.StageLogIndexCfg(db, cfg.Prune, dirs.Tmp), stagedsync.StageCallTracesCfg(db, cfg.Prune, 0, dirs.Tmp), stagedsync.StageTxLookupCfg(db, cfg.Prune, dirs.Tmp, snapshots, isBor, sprint), stagedsync.StageFinishCfg(db, dirs.Tmp, headCh, forkValidator), runInTestMode),
