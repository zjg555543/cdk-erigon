...
func (dc *DomainContext) Unwind(ctx context.Context, rwTx kv.RwTx, step, txNumUnwindTo, txNumUnwindFrom, limit uint64, f func(step uint64, k, v []byte) error) error {
	d := dc.d
	...
	for k, v, err = keysCursor.First(); err == nil && k != nil; k, v, err = keysCursor.Next() {
		...
		toRestore, needDelete, err := dc.hc.ifUnwindKey(k, txNumUnwindTo-1, rwTx)
		if err != nil {
	...
	defer logEvery.Stop()
	if err := dc.hc.Prune(ctx, rwTx, txNumUnwindTo, txNumUnwindFrom, limit, logEvery); err != nil {
		return fmt.Errorf("prune history at step %d [%d, %d): %w", step, txNumUnwindTo, txNumUnwindFrom, err)
	}
