...
func (s *ClientSynchronizer) syncBlocks(dbTx kv.RwTx, lastEthBlockSynced *state.Block) (*state.Block, error) {
	...
	*/
	...
	if lastEthBlockSynced.BlockNumber > 16920164 {
...
func (s *ClientSynchronizer) processBlockRange(dbTx kv.RwTx, blocks []etherman.Block, order map[common.Hash][]etherman.Order) error {
	...
	for i := range blocks {
		...
		if err != nil {
			...
			//rollbackErr := dbTx.Rollback(s.ctx)
			if err != nil {
				log.Errorf("error rolling back state to store block. BlockNumber: %d, rollbackErr: %s, error : %v", blocks[i].BlockNumber, err.Error(), err)
				return err
			}
			return err
		}
